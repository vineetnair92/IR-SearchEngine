...
def main(args):
    ...
    for topic in sorted(trec):
		...
		avg_prec= sum_prec/float(num_rel[topic])
		    ...
		    f1.append(2*prec_list[cutoff]*rec_list[cutoff]/float(prec_list[cutoff]+rec_list[cutoff]))
		#print(f1)
		rec_at_cutoffs= []
		for cutoff in cutoffs:
		...
		if(check):
		    eval_print(topic,num_ret,num_rel[topic],num_rel_ret,prec_at_recalls,avg_prec,prec_at_cutoffs,rec_at_cutoffs,r_prec,ndcg,f1)
    ...
    for i in range(0,len(cutoffs)):
	    ...
	    avg_f1[i]=sum_f1[i]/float(num_topics)
    ...
    eval_print(num_topics,tot_num_ret,tot_num_rel,tot_num_ret_rel,avg_prec_at_recalls,mean_avg_prec,avg_prec_at_cutoffs,avg_rec_at_cutoffs,avg_r_prec,mean_dcg,avg_f1)




def eval_print(a,s,d,f,g,h,j,l,k,m,n):
  ...
  print ("F1:")
  print ("  At    5 docs:   %.4f"% float(n[0]))
  print ("  At   10 docs:   %.4f"% float(n[1]))
  print ("  At   20 docs:   %.4f"% float(n[2]))
  print ("  At   50 docs:   %.4f"% float(n[3]))
  print ("  At   100 docs:   %.4f"% float(n[4]))
